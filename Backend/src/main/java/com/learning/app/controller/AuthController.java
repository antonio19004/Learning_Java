	...
	private ResponseEntity<?> authentication(@RequestBody AuthenticationRequest authRequest){
		...
		try {
			...
			if ("ROLE_ADMIN".equals(rol)) {
				return ResponseEntity.ok(new AuthenticationResponse("Autenticación Exitosa - Admin " + user ));
			} else {
				return ResponseEntity.ok(new AuthenticationResponse("Autenticación Exitosa " + user ));
			}
		} catch (Exception e) {
